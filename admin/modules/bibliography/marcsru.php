...
if (isset($_POST['saveZ']) AND isset($_SESSION['marcresult'])) {
  ...
  foreach ($_POST['zrecord'] as $id) {
    ...
    if ($a) {
      // set title
      $data['title'] = preg_replace('{/$}', '', addslashes($a->getSubfield('a') . $a->getSubfield('b') . $a->getSubfield('l')));
      // set SOR
      $data['sor']   = ($a->getSubfield('c')) ? $a->getSubfield('c') : '';
      // set GMD
      $data['gmd_id'] = 1;
      if ($a->getSubfield('h')) {
        $gmd_name = ucwords(trim(preg_replace('/[^a-z0-9S]/i',' ',$a->getSubfield('h'))));
        $data['gmd_id'] = utility::getID($dbs, 'mst_gmd', 'gmd_id', 'gmd_name', $gmd_name, $gmd_cache);
      }
    }
    // set edition
    ...
    if ($h) {
      $language_name = strtolower($h->getSubfield('a'));
      $data['language_id'] = utility::getID($dbs, 'mst_language', 'language_id', 'language_id', $language_name, $lang_cache);
    }
    // set notes
    $i = $record->getField(520);
    if ($i) {
...
if (isset($_GET['keywords'])) {
  ...
  if ($marc->count() > 0) {
    ...
    for ($i=1; $i <= $marc->count(); $i++) {
      ...
      //cover
      $rAttach = $record->getField('856');
      $img = '../images/default/image.png';   
      if($rAttach){
        if($rAttach->getSubfield('x')){
           $img = $rAttach->getSubfield('x');
        }
      }
      ...
      echo '<td width="2%" class="'.$row_class.'"><img src="'.$img.'" style="max-width:135px;"/></td>';
      echo '<td width="78%" class="'.$row_class.'"><strong>'.$title.'</strong><div><i>'.$authors.'</i></div></td>';
      if ($rISBN) {
